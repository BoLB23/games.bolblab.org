...
COPY games/flappy-mike/package.json games/flappy-mike/package.json
...
COPY games/flappy-mike games/flappy-mike
...
RUN npm run build --workspace @game-platform/game-client-sdk \
    ...
    && VITE_API_BASE_URL="$VITE_API_BASE_URL" VITE_CATALOG_URL="$VITE_CATALOG_URL" \
      npm run build --workspace @game-platform/flappy-mike -- --base=/games/flappy-mike/
...
COPY --from=build /app/games/flappy-mike/dist /usr/share/nginx/html/games/flappy-mike
